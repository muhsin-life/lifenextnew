...
const buttonVariants = cva(
  "inline-flex items-center justify-center  duration-200 transition-all active:scale-95",
  {
    variants: {
      variant: {
        default: cn(
        ...
        themeToggleBtn:
          "bg-white text-life border border-white ",
      },
      size: {
        ...
        xs: cn(typographyVariants({ size: "xs" }), "px-2 py-[2px]"),
      },
      position:{
        inputLeftBtn: "flex items-center  ltr:rounded-r-none rtl:rounded-l-none",
        inputRightBtn: "flex items-center  ltr:rounded-l-none rtl:rounded-r-none",
      },
      rounded: {
        full: "rounded-full",
        md: "rounded-md",
    ...
    return (
      ...
      >
        {isLoading ? null : iconLeft ? (
          <Icon type={iconType} sizes={size} className={`rtl:ml-1 ltr:mr-1`} />
        ) : null}
        {isLoading ? (
          <Icon
            type="refreshIcon"
